var slide = {
    'delete': function(id) {
        $('#href_delete').attr('href', '/admin/slide/delete/'+id);
        $('#modal_delete_slide').modal('show');
    }
};
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.errors != undefined) {
                    ...
                    callToast.error('Ошибка', 'при добавлении слайда');
                    return false;
                }
                if (response.status == 'success') {
                    $('#image').val(null);

                    $('#active_image').attr('src', '/assets/images/slides/'+response.image);
                    $('#block_active_image').show();

                    itemId.attr('value', response.item_id);
                    $('#btn_create').hide();
                    $('#btn_update').show();
                    callToast.success('Слайд', 'успешно добавлен');
                }
            }
        });
    });
    ...
    $('#btn_update').click(function() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.errors != undefined) {
                    insertErrorArray(form, response.errors, group, errorList);
                    callToast.error('Ошибка', 'при обновлении данных о слайде');
                    return false;
                }
                if (response.status == 'success') {
